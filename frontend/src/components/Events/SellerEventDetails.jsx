    ...
    <div className="bg-[#fff] shadow-xl">
      {event ? (
        <div className="fixed w-full h-screen top-0 lg:top-0 left-0 bg-[#00000050] z-50 flex items-center justify-center shadow-xl">
          <div className="w-[90%] lg:w-[60%] h-[80vh] md:h-[90vh] overflow-y-scroll scrollbar-none lg:h-[75vh] bg-white rounded-md shadow-2xl relative p-2 mt-[82px] ">
                    ...
                    {event.images?.map((photo, index) => (
                      ...
                      >
                        <img
                          src={`${photo?.url}`}
                          alt={`pics ${index + 1}`}
                          className="max-w-[150px]  lg:max-w-[90%] min-h-[200px] lg:h-[50%] object-contain"
                        />
                        <div
